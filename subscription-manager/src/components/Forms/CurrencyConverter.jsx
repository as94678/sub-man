// src/components/Forms/CurrencyConverter.jsx

import React from 'react';
import { CURRENCIES } from '../../utils/currency';

const CurrencyConverter = ({ exchangeRates, darkMode, isVisible }) => {
  if (!isVisible) return null;

  return (
    <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-md`}>
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-3 sm:py-4">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
          {CURRENCIES.map(currency => (
            <div key={currency} className="text-center p-2 rounded-lg bg-opacity-50 hover:bg-opacity-70 transition-colors">
              <div className={`text-xs sm:text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>{currency}</div>
              <div className="font-semibold text-sm sm:text-base">
                {currency === 'TWD' ? '1.00' : exchangeRates[currency].toFixed(2)}
              </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default CurrencyConverter;